...
return_type aes_encrypt16( const uint8_t in[N_BLOCK], uint8_t  out[N_BLOCK], const aes_context ctx[1] )
{
    if( ctx->rnd )
    {
                         ...
                         int32_t n_block, uint8_t iv[N_BLOCK], const aes_context ctx[1] )
{
    ...
    {
        xor_block(iv, in);
        if(aes_encrypt16(iv, iv, ctx) != EXIT_SUCCESS)
            return EXIT_FAILURE;
...
static return_type aes_decrypt16( const uint8_t in[N_BLOCK], uint8_t out[N_BLOCK], const aes_context ctx[1] )
{
    if( ctx->rnd )
    {
        uint8_t s1[N_BLOCK], r;
